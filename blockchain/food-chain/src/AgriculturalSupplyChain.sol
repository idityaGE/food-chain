...
contract AgriculturalSupplyChain {
    ...
    modifier onlyBatchOwner(uint256 _batchId, address _owner) {
        require(
            productBatches[_batchId].currentOwner == _owner,
            "Not the batch owner"
        );
        _;
    }
    ...
    function createProductBatch(
        ...
        stakeholderBatches[_farmerAddress].push(batchId);

        emit ProductBatchCreated(batchId, _farmerAddress, _productType);
        return batchId;
    }
    ...
    function transferBatch(
        ...
        // Record transaction
        batchTransactions[_batchId].push(
            Transaction({
                from: previousOwner,
                to: _to,
    ...
    ) external onlyBatchOwner(_batchId, _owner) batchExists(_batchId) {
    ...
    function getBatchTransactions(
        uint256 _batchId
    ) external view batchExists(_batchId) returns (Transaction[] memory) {
    ...
    function isVerifiedStakeholder(
        address _stakeholder
